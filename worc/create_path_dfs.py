...
def get_mask_suffix(subject):
    if subject.startswith("CRLM"): 
        return "_lesion0_RAD"
    elif subject.startswith("Melanoma") or subject in subjects_with_two_segmentations:
        return "_lesion0"
    return ""
...
def create_path_df_for_single_dataset(dataset):    
    ...
    result_dict = {"subject_id": [], "label": [], "image_path": [], "mask_path": []}
    for _, row in df_dataset.iterrows():
        subject = row['Subject']
        label = row["Diagnosis_binary"]
        modality = get_modality(row)
        
        dir_path = config.WORC_DIR / "data" / (f"{subject}_{modality}") / "1" / "NIFTI"
    ...
    df_result.to_csv(config.WORC_DIR / "tables" / "derived" / f"{dataset}_paths.csv", index=False)
